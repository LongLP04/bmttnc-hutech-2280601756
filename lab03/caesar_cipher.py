...
class MyApp(QMainWindow):
    ...
    def call_api_encrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.ui.textEdit_2.setPlainText(data["encrypted_message"])  # Fixed to setPlainText
                msg = QMessageBox()
    ...
    def call_api_decrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.ui.textEdit.setPlainText(data["decrypted_message"])  # Fixed to setPlainText
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Information)  # Corrected the message box icon
